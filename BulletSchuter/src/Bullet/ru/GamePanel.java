...
public class GamePanel extends JPanel implements Runnable, KeyListener{
	...
	private void gameUpdate(){
		...
		//create enemies (создавать врагов)
		...
		//enemy update (обновление врага)
		for(int i = 0; i < enemies.size(); i++){
			enemies.get(i).update();
		}
		
		//powerup update (Обновление питания)
		...
		//explosion update (обновление взрыва)
		...
		//text update (текстовое обновление)
		...
		//bullet-enemy collision (столкновение пули с врагом)
		for(int i = 0; i < bullets.size(); i++){
		...
		//check dead enemies (проверять мертвых врагов)
		for(int i = 0; i < enemies.size(); i++){
			if(enemies.get(i).isDead()){
				Enemy e = enemies.get(i);
				
				//chance for powerup (шанс для питания)
		...
		//check dead player (проверить мертвого игрока)
		if(player.isDead()){
			running = false;
		}
		
		//player-enemy collision (столкновение игрока с врагом)
		...
		//player - powerup collision (игрок - силовой удар)
		int px = player.getx();
		int py = player.gety();
		int pr = player.getr();
		for(int i = 0; i < powerups.size(); i++){
			...
			//collected powerup (собранная мощность)
			if(dist < pr + r){
		...
		//slowdown update (замедление обновления)
		if(slowDownTimer != 0){
	...
	private void gameRender(){
		//draw background (рисовать фон)
		g.setColor(new Color(0, 100, 255));
		g.fillRect(0, 0, WIDTH, HEIGHT);
		
		//draw slowdown screen (трафик замедления)
		if(slowDownTimer != 0){
		...
		//draw player (привлечь игрока)
		player.draw(g);
		
		//draw bullet (рисовать пулю)
		for(int i = 0; i < bullets.size(); i++){
			bullets.get(i).draw(g);
		}
		
		//draw enemy (привлекать врага)
		for(int i = 0; i < enemies.size(); i++){
			enemies.get(i).draw(g);
		}
		
		//draw powerup (рисовать powerup)
		for(int i = 0; i < powerups.size(); i++){
			powerups.get(i).draw(g);
		}
		
		//draw explosions (взрывать взрывы)
		for(int i = 0; i < explosions.size(); i++){
		...
		//draw wave number (количество волновых чисел)
		if(waveStartTimer != 0){
		...
		//draw player live (привлечь игрока вживую)
		...
		//draw player power (привлекать игрока)
		...
		//draw player score (набрать счет игрока)
		g.setColor(Color.WHITE);
		g.setFont(new Font("Century Gothic", Font.PLAIN, 14));
		g.drawString("Score: " + player.getScore(), WIDTH - 100, 30);
		
		
		//draw slowdown meter (счетчик замедления)
		if(slowDownTimer != 0){
			g.setColor(Color.WHITE);
